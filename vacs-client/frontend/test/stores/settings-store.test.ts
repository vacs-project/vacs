...
const RESPONSES: Record<string, unknown> = {
    ...
    playback_get_enabled: false,
    playback_get_say_again: true,
};
...
describe("settings store", () => {
    it("fetchSettings loads the say again setting", async () => {
        invoke.mockImplementation((cmd: string) => Promise.resolve(RESPONSES[cmd]));

        await fetchSettings();

        const state = useSettingsStore.getState();
        expect(invoke).toHaveBeenCalledWith("playback_get_say_again", undefined);
        expect(state.sayAgainEnabled).toBe(true);
        expect(state.playbackEnabled).toBe(false);
        expect(state.radioConfig).toEqual(RADIO_CONFIG);
        expect(state.transmitConfig?.pushToTalkLabel).toBe("A");
